...
| P9 | `public-workspace-branding.patch` (**não planejado** — decisão de trabalho OQ-12-1: endpoint separado; mantido só como opção) | `brandingHash` na `PublicWorkspaceDataDTO` | `public-workspace-data.dto.ts`, `workspace.resolver.ts` | médio | módulo server | integração resolver | reavaliar contra mudanças do resolver |
